...
기준은 이렇게 잡았다.
`유효 / 확인됨 / OPEN`은 **문제 있음**, `해결 / 비유효 / RESOLVED`는 **문제 아님**, `부분 유효 / 부분확인`은 **의견 갈림**으로 분류했다.

## 1) 세 보고서 전부 “문제 있다”고 본 사항

총 **19건**

1. **#1 드래프트 combine 경로 오류**
2. **#2 만료 계약 선수 방출 버튼 실패**
3. **#3 워크아웃 추가 진행 교착**
4. **#4 standalone roster import / `season_year` SSOT 예외 경로**
5. **#6 Player Training 선수 전환 race / 오저장 가능성**
6. **#10 ACTIVE 협상 세션 상한 미적용**
7. **#12 `overpay_allowed` vs `probe_max` 스케일 불일치**
8. **#13 세션 재사용 시 `created_at` 미갱신**
9. **#14 `buy_target_basketball_norm_mode="FIXED"` 무시**
10. **#16 `PackageEffectsConfig` 레거시 ctor/키 비호환**
11. **#19 preset override label alias 미처리**
12. **#20 `skeleton_overhaul_enabled` 사실상 미사용**
13. **#21 `template_only` + 빈 route = 전체 허용**
14. **#22 template wrapper `expected_tag` 하드코딩**
15. **#24 pick protection이 sweetener gate에 종속**
16. **#28 Trade Lab 팀 자산 API 픽 필터 약함**
17. **#29 `renderAssetList` stored XSS**
18. **#33 tier 분류가 league-rank 전체 snapshot hot path 의존**
19. **#34 soft-boundary `exp` overflow guard 부재**

---

## 2) 세 보고서 전부 “지금은 문제 아니다”고 본 사항
...
### #7 숫자 정렬 CSS 오염 / Trade Block 정렬 문제

* **1번:** 해결/비유효
* **2번:** 확인됨(유효)
* **3번:** OPEN

### #9 `_emit` noise floor 약화

* **1번:** 부분 유효
* **2번:** 부분확인
* **3번:** OPEN

### #11 REJECT → LOWBALL 과승격

* **1번:** 부분 유효
* **2번:** 확인됨(유효)
* **3번:** OPEN

### #17 outgoing 후보에 비전송 자산 포함

* **1번:** 유효
* **2번:** 부분확인
* **3번:** RESOLVED/OUTDATED

### #18 greedy fill / 고가치 자산 선소모 편향

* **1번:** 유효
* **2번:** 부분확인
* **3번:** OPEN

### #23 template-first 비활성 / combined fallback 경로

* **1번:** 부분 유효
* **2번:** 해결됨/비유효
* **3번:** OPEN

### #25 protection variant 평가에서 repeat penalty 무시

* **1번:** 유효
* **2번:** 부분확인
* **3번:** OPEN

### #26 repaired variant 루프 내부 예산/하드캡 재검사 누락

* **1번:** 유효
* **2번:** 부분확인
* **3번:** OPEN

### #30 `LockInfo/lock` surface 제거 호환성

* **1번:** 부분 유효
* **2번:** 부분확인
* **3번:** RESOLVED/OUTDATED

### #31 bubble bonus / bubble 조건 문제

* **1번:** 해결/비유효
* **2번:** 해결됨/비유효
* **3번:** OPEN

### #32 blended score 기준치 / rank-soft threshold 문제
...
* **합의가 가장 강한 현재 이슈**는 **19건**
* **이미 정리된 것으로 보는 항목**은 **4건**
* 나머지 **11건은 재현 조건, 코드 경로, 테스트 해석 차이 때문에 의견이 갈린 상태**다.
